        ...
        if (data.type == 'pokemon' || data.type == 'raid' || data.type == 'quest') {
            ...
            MAIN.Discord.Servers.forEach(async(server, index) => {
                ...
                if (InsideGeojson.polygon(server.geofence, [data.message.longitude, data.message.latitude])) {
                    ...
                    let timezone = GeoTz(server.geofence[0][0][1], server.geofence[0][0][0])[0];
                    discord_match = true;
                    ...
                    await MAIN.geofences.features.forEach(async(geofence, index) => {
                        if (InsideGeojson.polygon(geofence.geometry.coordinates, [data.message.longitude, data.message.latitude])) {
                            if (geofence.properties.sub_area != 'true') { geofence_area.main = geofence.properties.name; } else if (geofence.properties.sub_area == 'false') { geofence_area.sub = geofence.properties.name; }
                        }
                    });

                    // ASSIGN AREA TO VARIABLES
                    if (geofence_area.sub) {
                        embed_area = geofence_area.sub;
                        sub_area = geofence_area.sub;
                    } else { embed_area = geofence_area.main; }
                    if (geofence_area.main) { main_area = geofence_area.main; } else {
                        embed_area = server.name;
                        main_area = server.name;
                    }
                    ...
                    switch (data.type) {
                        ...
                        case 'pokemon':
                            Pokemon.run(MAIN, data.message, main_area, sub_area, embed_area, server, timezone);
                            break;
                            // SEND TO RAIDS MODULE
                        case 'raid':
                            Raids.run(MAIN, data.message, main_area, sub_area, embed_area, server, timezone);
                            break;
                            // SEND TO QUESTS MODULE
                        case 'quest':
                            Quests.run(MAIN, data.message, main_area, sub_area, embed_area, server, timezone);
                            break;
                    }
                }
            });
...
MAIN.Save_Sub = (message, server) => {
    MAIN.database.query(`SELECT * FROM pokebot.info`, function(error, info, fields) {
        ...
        MAIN.database.query(`INSERT INTO pokebot.users (user_id, user_name, geofence, pokemon, quests, raids, status, bot, alert_time, discord_id, pokemon_status, raids_status, quests_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [message.member.id, message.member.user.tag, server.name, , , , 'ACTIVE', next_bot, MAIN.config.QUEST.Default_Delivery, message.guild.id, 'ACTIVE', 'ACTIVE', 'ACTIVE'], function(error, user, fields) {
            if (error) { console.error('[Pokébot] [' + MAIN.Bot_Time(null, 'stamp') + '] UNABLE TO ADD USER TO pokebot.users', error); } else {
...
MAIN.Get_Sprite = (form, id) => {
    ...
    switch (id.toString().length) {
        case 1:
            sprite_url = 'https://www.serebii.net/sunmoon/pokemon/00' + id + '.png';
            break;
        case 2:
            sprite_url = 'https://www.serebii.net/sunmoon/pokemon/0' + id + '.png';
            break;
        case 3:
            sprite_url = 'https://www.serebii.net/sunmoon/pokemon/' + id + '.png';
            break;
    }
    // CHECK FOR ALOLAN
...
MAIN.Send_Embed = (embed, channel_id) => {
    ...
    return MAIN.BOTS[MAIN.Next_Bot].channels.get(channel_id).send(embed).then(
        (message) => {
            message.edit(`ID=${bigInt(message.id).toString(36)}?${bigInt(message.channel.id).toString(36)}!`, { embed: message.embeds[0] }).catch(
                error => {
                    console.error(`[${channel_id}]`, error)
                }
            )
        }
    ).catch(error => {
...
MAIN.Static_Map_Tile = (lat, lon, type) => {
    return new Promise(async function(resolve, reject) {
        ...
        if (fs.existsSync(path)) { resolve(url); /*console.error('[Pokébot] ['+MAIN.Bot_Time(null,'stamp')+'] USED AN EXISTING MAP TILE FOR '+lat+','+lon+'.');*/ } else {
            const zoom = 16,
                center = [lon, lat],
                options = { width: 400, height: 220 };
            const map = new StaticMaps(options);
            const marker = { img: `https://i.imgur.com/OGMRWnh.png`, width: 40, height: 40 };
            marker.coord = [lon, lat];
            map.addMarker(marker);
            await map.render(center, zoom)
                .then(async() => {
                    await map.image.save('./static/images/' + type + '_tiles/' + lat + ',' + lon + '.png');
                    resolve(url);
                })
...
MAIN.Get_Type = (variable) => {
    return new Promise(resolve => {
        switch (MAIN.moves[variable].type) {
            case 'Normal':
                emote = MAIN.emotes.normal;
                break;
            case 'Grass':
                emote = MAIN.emotes.grass;
                break;
            case 'Fire':
                emote = MAIN.emotes.fire;
                break;
            case 'Water':
                emote = MAIN.emotes.water;
                break;
            case 'Electric':
                emote = MAIN.emotes.electric;
                break;
            case 'Ground':
                emote = MAIN.emotes.ground;
                break;
            case 'Steel':
                emote = MAIN.emotes.steel;
                break;
            case 'Rock':
                emote = MAIN.emotes.rock;
                break;
            case 'Psychic':
                emote = MAIN.emotes.psychic;
                break;
            case 'Poison':
                emote = MAIN.emotes.poison;
                break;
            case 'Fairy':
                emote = MAIN.emotes.fairy;
                break;
            case 'Fighting':
                emote = MAIN.emotes.fighting;
                break;
            case 'Dark':
                emote = MAIN.emotes.dark;
                break;
            case 'Ghost':
                emote = MAIN.emotes.ghost;
                break;
            case 'Bug':
                emote = MAIN.emotes.bug;
                break;
            case 'Dragon':
                emote = MAIN.emotes.dragon;
                break;
            case 'Ice':
                emote = MAIN.emotes.ice;
                break;
            case 'Flying':
                emote = MAIN.emotes.flying;
                break;
            default:
                emote = '';
        }
...
async function update_each_version(version) {
    return new Promise(async(resolve) => {
        for (let u = version; u <= MAIN.db.LATEST; u++) {
            if (u == MAIN.db.LATEST) { resolve('DONE'); } else {
                let updateTo = u + 1;
...
function clear_channel(channel_id) {
    return new Promise(function(resolve) {
        ...
        if (!channel) {
            resolve(false);
            console.error("Could not find a channel with ID: " + channel_id);
            return;
        }
...
MAIN.start = async(type) => {
    ...
    switch (type) {
        case 'startup':
            ...
            break;
        case 'reload':
            console.log('[Pokébot] [' + MAIN.Bot_Time(null, 'stamp') + '] [Re-Load] Pokébot has re-loaded.');
            break;
    }
}
MAIN.start('startup');